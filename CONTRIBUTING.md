...
Output is Vehicle-MultiSelect-v0.13.zip. Add `--noimages` to build Vehicle-MultiSelect-v0.13-noimages.zip. Both use the same game resources. The builder needs no earlier release ZIP. Maintain source directly in this repository; no source ZIP is needed.

## Repository layout
- src/: Lua runtime, guarded data changes and deployed-option detection.
- config/: supported game hashes and expected record flags.
- scripts/: build.py is the release entry point; runtime.py assembles the runtime; archive_format.py writes game archives.
...
## Working directories
The active local checkout is mods/VehicleMultiSelect and its origin is https://github.com/Toritte/VehicleMultiSelect.
Build into dist/ (ignored by Git). Keep published release ZIPs outside this repository in the workspace releases directory. The existing VehicleMultiSelectStandalone release path is retained as a historical storage location.
Use docs/RELEASE_NOTES.md for the changelog. Keep repository history and author-edited README text when synchronizing releases. Do not copy research, extracted game resources, historical candidate builders or source ZIPs into GitHub.
