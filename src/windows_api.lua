return function()
    ...
    function api.writable_data(address, size)
        ...
        local region = ffi.new('BsbMemoryRegion[1]')
        -- FFI declarations are shared with other mods that may name this layout differently.
        local region_buffer = ffi.cast('void *', region)
        while remaining > 0 do
            if kernel.VirtualQuery(cursor, region_buffer, ffi.sizeof(region[0])) ~= ffi.sizeof(region[0]) then return false end
